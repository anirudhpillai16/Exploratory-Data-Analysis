...
##Goals achieved by Re-expressing data##
...
When g = 0, h=0 Gaussian Data , No skweness and No long tails.
...
#Kolmogorov Test
* A feature of this test is that distribution of the K-S test statistics itself does not depend on underlying cumulative
...
Poissonness plot, we may want to judge more formally whether it is unlikely to have done so by chance. 
...
boot<-function(matrix,n){
  ...
  for (j in 1:n){
    ...
    combinedResults<-rbind(temp.result$row,temp.result$row,temp.result$row,temp.result$row,temp.result$row,
    ...
    bs.matrix <- new_res+t(combinedResults)+
              sapply(c(temp.result$col),function(x) rep(x,nrow(temp.result$residuals)))+matrix(temp.result$overall,
